...
type SimpleCaseList =
    { Name: string; Cases: SimpleCase list }
...
let simpleCase<'T when 'T: equality> name (instance: 'T) =
    { Name = name
      Comparer = (fun msg v1 v2 -> Expect.equal msg (v1 :?> 'T) (v2 :?> 'T))
      Instance = instance
      InstanceType = typeof<'T> }
...
let compareSequences<'T> msg (expected: obj) (actual: obj) =
    Expect.isTrue msg
    <| System.Linq.Enumerable.SequenceEqual((expected :?> IEnumerable<'T>), (actual :?> IEnumerable<'T>))

let compareDictionaries<'TKey, 'TValue> msg (expected: obj) (actual: obj) =
    compareSequences<KeyValuePair<'TKey, 'TValue>>
        msg
        ((expected :?> Dictionary<'TKey, 'TValue>).OrderBy(fun kv -> kv.Key).ToList())
        ((actual :?> Dictionary<'TKey, 'TValue>).OrderBy(fun kv -> kv.Key).ToList())

let evolutionCase<'TSource, 'TDest when 'TDest: equality>
    name
    (instance: 'TSource)
    (expectedInstance: 'TDest)
    (annotations: string)
    =
    { Name = name
      Comparer = (fun msg v1 v2 -> Expect.equal msg (v1 :?> 'TDest) (v2 :?> 'TDest))
      Instance = instance
...
let simpleCases =
    [ simpleCaseList
      ...
      simpleCaseList "Enum" [ simpleCase "SimpleEnum" TestState.Green ]
      simpleCaseList
          "Array"
      ...
      simpleCaseList "CustomTypes" [ simpleCase "Uri" { Value = System.Uri("http://www.example.com") } ] ]
...
let evolutionCases =
    ...
    "records": [
      ...
      |> evolutionCase
          "Added string field"
          ({ Id = 456 }: RecordWithId)
          ({ Id = 456; NewField = "Hello" }: RecordWithNewField)
    ...
    "records": [
      ...
      |> evolutionCase
          "New Record"
          ({ Id = 456; Title = "Hello World!!!" }: OldRecord)
          ({ Id = 456
             Caption = "Hello World!!!"
    ...
    "enums": [
      ...
      |> evolutionCase
          "UnknownUnion in Record"
          ({ Union = Case3 "Hello!" }: RecordV2)
          ({ Union = UnionV1.UnknownCase "" }: RecordV1)
      ...
      |> evolutionCase
          "Unknown Union field in Record"
          ({ Union = Case3 "Hello!" }: RecordV2)
          ({ Union = Case3 "Hello!"
             Union2 = UnknownCase "World!" }
          : RecordV3)
      ...
      |> evolutionCase
          "Map with unknown case"
          ([ "I1", Case1 "Start"; "I2", Case3 "Hello!"; "I3", Case2; "I4", Case1 "End" ]
           |> Map.ofList)
          ([ "I1", UnionV1.Case1 "Start"
             "I2", UnionV1.UnknownCase ""
             "I3", UnionV1.UnknownCase ""
             "I4", UnionV1.Case1 "End" ]
           |> Map.ofList)
    ...
    "records": [
      ...
      |> evolutionCase
          "Default value for custom rule type"
          { Value = System.Uri("http://www.example.com") }
          { Value = System.Uri("http://www.example.com")
            NewValue = System.Uri("about:blank") }
...
let json2SimpleTest (case: SimpleCase) =
    test case.Name {
        ...
        let serializer =
            Serde.jsonSerializer Serde.defaultSerializerOptions (case.InstanceType) schema

        let deserializer =
            Serde.jsonDeserializer Serde.defaultDeserializerOptions (case.InstanceType) schema
...
let json2EvolutionTest (case: EvolutionCase) =
    test case.Name {
        ...
        let serializer =
            Serde.jsonSerializer Serde.defaultSerializerOptions (case.InstanceType) writerSchema
        ...
        let readerSchema =
            match
                Schema.generate
                    ...
                    (case.ExpectedInstance.GetType())
            with
            | Ok schema -> schema
            | Error err -> failwithf "Schema error: %A" err

        let deserializer =
            Serde.jsonDeserializer Serde.defaultDeserializerOptions (case.ExpectedInstance.GetType()) readerSchema
...
let binary2SimpleTest (case: SimpleCase) =
    test case.Name {
        ...
        let serializer =
            Serde.binarySerializer Serde.defaultSerializerOptions (case.InstanceType) schema

        let deserializer =
            Serde.binaryDeserializer Serde.defaultDeserializerOptions (case.InstanceType) schema
...
let binary2EvolutionTest (case: EvolutionCase) =
    test case.Name {
        ...
        let serializer =
            Serde.binarySerializer Serde.defaultSerializerOptions (case.InstanceType) writerSchema
        ...
        let deserializer =
            Serde.binaryDeserializer Serde.defaultDeserializerOptions (case.ExpectedInstance.GetType()) readerSchema
...
let json2EvolutionTests =
    evolutionCases |> List.map json2EvolutionTest |> testList "Json2Evolution"
...
let binary2EvolutionTests =
    evolutionCases |> List.map binary2EvolutionTest |> testList "Binary2Evolution"
...
let leaveStreamOpenTests =
    test "Serde should leave its stream open" {
        ...
        let serializer =
            Serde.binarySerializer Serde.defaultSerializerOptions subjectType schema

        let deserializer =
            Serde.binaryDeserializer Serde.defaultDeserializerOptions subjectType schema
