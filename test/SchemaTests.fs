...
let generateSchemaWithAnnotation type' ann =
    Schema.generate {Schema.defaultOptions with Annotations = ann} type'

let genSchemaWithStubs type' =
    Schema.generate {Schema.defaultOptions with StubDefaultValues = true} type'
...
let logicalTypesTests =
    testList "LogicalTypes" [
        ...
        test "DecimalAsDouble" {
            Schema.generate {Schema.defaultOptions with TreatDecimalAsDouble = true} typeof<decimal>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type": "double"}"""
        }
        test "Guid" {
        ...
        test "GuidAsString" {
            Schema.generate {Schema.defaultOptions with TreatGuidAsString = true} typeof<System.Guid>
            |> Expect.wantOk "Schema should be created"
        ...
        test "TimeSpan" {
            ...
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type": "int"}"""
        }
        test "DateTimeOffset" {
        ...
        test "UriInGenericRecord" {
            ...
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Uri","fields":[{"name":"Value","type":"string"}]}"""
        }
    ]|> testLabel "Schema"
...
let stubTest =
    testList "Stub Default Values" [
        test "String" {
            ...
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_String","fields":[{"name":"Value","type":"string","default":""}]}"""
        }
        test "Bool" {
            genSchemaWithStubs typeof<GenericRecord<bool>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Boolean","fields":[{"name":"Value","type":"boolean","default":false}]}"""
        }
        test "Int" {
            genSchemaWithStubs typeof<GenericRecord<int>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Int32","fields":[{"name":"Value","type":"int","default":0}]}"""
        }
        test "Decimal" {
            genSchemaWithStubs typeof<GenericRecord<decimal>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Decimal","fields":[{"name":"Value","type":{"type":"bytes","logicalType":"decimal","precision":29,"scale":14},"default":""}]}"""
        }
        test "DecimalAsDouble" {
            Schema.generate {Schema.defaultOptions with StubDefaultValues = true; TreatDecimalAsDouble = true} typeof<GenericRecord<decimal>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Decimal","fields":[{"name":"Value","type":"double","default":0}]}"""
        }
        test "Bytes" {
            genSchemaWithStubs typeof<GenericRecord<byte []>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Array_Of_Byte","fields":[{"name":"Value","type":"bytes","default":""}]}"""
        }
        test "List" {
            genSchemaWithStubs typeof<GenericRecord<string list>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Array_Of_String","fields":[{"name":"Value","type":{"type": "array","items": "string"},"default":[]}]}"""
        }
        test "Map" {
            genSchemaWithStubs typeof<GenericRecord<Map<string,string>>>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Map_Of_String","fields":[{"name":"Value","type":{"type": "map", "values": "string"},"default":{}}]}"""
        }
        test "Enum" {
            genSchemaWithStubs typeof<TestState>
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type": "enum", "name": "Foo.Bar.TestState", "symbols": ["Green", "Yellow", "Red"], "default":"Green"}"""
        }
        test "SimpleRecord" {
        ...
        test "UriInGenericRecord" {
            ...
            |> Expect.wantOk "Schema should be created"
            |> expectSchemasEqual <| Schema.ofString """{"type":"record","name":"Foo.Bar.GenericRecord_Of_Uri","fields":[{"name":"Value","type":"string", "default":""}]}"""
        }
